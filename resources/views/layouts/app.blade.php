        ...
        <aside class="w-68 sidebar-gradient text-white hidden md:flex md:flex-col flex-shrink-0 z-30 shadow-2xl">
                ...
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 group">
                    ...
                    <div class="flex flex-col">
                        <span class="text-lg font-extrabold tracking-wider text-white leading-none">
                            Quick<span class="text-blue-400">.Deal</span>
                        </span>
                ...
                <div x-data="{ open: false }">
                    ...
                    <div id="penawaran-submenu" class="submenu space-y-1">
                        <a href="{{ route('penawaran.create_combined') }}" class="flex items-center px-4 py-2.5 text-sm text-slate-400 hover:text-white hover:translate-x-1 transition-transform tracking-wide italic">
                            <span class="mr-2 text-blue-500">•</span> Penawaran Jasa
                        </a>
            ...
            <div class="p-4 m-4 bg-slate-900/50 rounded-2xl border border-slate-700/50">
                <div class="flex items-center gap-3">
                    <a href="{{ route('profile.edit') }}" class="h-10 w-10 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold shadow-lg shadow-blue-500/20 hover:scale-105 transition-transform" title="Buka Pengaturan Akun">
                        {{ substr(Auth::user()->name ?? 'U', 0, 1) }}
                    </a>
                    <div class="text-sm flex-1 min-w-0">
                        <a href="{{ route('profile.edit') }}" class="font-bold text-white truncate hover:underline block" title="Buka Pengaturan Akun">{{ Auth::user()->name ?? 'Guest User' }}</a>
                    ...
                    <div class="flex items-center gap-0.5">
                        <a href="{{ route('profile.edit') }}" class="p-2 text-slate-400 hover:text-blue-400 hover:bg-blue-400/10 rounded-lg transition-all" title="Pengaturan Akun">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <form action="{{ route('logout') }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="p-2 text-slate-400 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all" title="Keluar">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
